...
//                .andExpect(jsonPath("$.price").value(price));
        ...
        when(productService.findProductById(id)).thenReturn(mockResponse);
        this.mockMvc.perform(get("/api/v1/products" + stringID))
                .andExpect(status().isOk()).andExpect(content().contentType(MediaType.APPLICATION_JSON))
                .andExpect(content().contentTypeCompatibleWith(MediaType.APPLICATION_JSON)) // Assert response content type
                .andExpect(content().contentType(MediaType.APPLICATION_JSON)).
                andExpect(jsonPath("$.name").value("lead")).
                andExpect(jsonPath("$.description").value("lead description")).
